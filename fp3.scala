...
object fp3 {
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    def helper [X] (xs:List[X], acc:Int) : List[(Int, X)] = {
		xs match {
			case Nil => Nil
			case y::Nil => (acc, y) :: Nil
			case y::ys => if (y == ys.head) helper(ys, acc + 1) else (acc, y) :: helper(ys, 1)
			}
			}
		helper(xs, 1)
  }
  ...
  def allDistinct (xs : List[Int]) : Boolean = {
     xs match {
		case Nil => true
		case y :: ys => if (member(y, ys) == true) false else allDistinct(ys)
		}
  }
}
