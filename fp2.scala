...
object fp2 {
  ...
  def joinTerminateRight (xs : List[String], term : String) : String = {
    def f (t : String, r : String) : String = term + t + r
	xs match {
		case Nil => ""
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
    def f (t : String, r : String) : String = t + r + term
		xs match {
			case Nil => ""
  ...
  def firstNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    xs match {
		...
		case y::ys => if (y < a) firstNumGreaterThan(a, ys) else y
			}
  }
  ...
  def firstIndexNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    xs.indexOf(aux1(a, xs))
		}
	def aux1(a : Int, xs : List[Int]) : Int = {
		xs match {
			case Nil => throw new java.util.NoSuchElementException
			case y::ys => if (y < a) aux1(a, ys) else y
			}
  }
}
